 ...
 //db opirations
  try {
   ...
   // creating new user and saving it to mongoDB using prisma
    ...
    console.log(err);
    res.status(500).json({ message: "Failed to create user!" });// error response
  }
};
...
export const login = async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(400).json({ message: "Invalid Credentials!" }); // error response (the numbers like 401 are error info for the web browser)
...
export const logout = (req, res) => {    //db opirations
